...
// Get ALL available sections from ALL year levels based on curriculum
$sections_sql = "SELECT s.section_id, s.section_code, s.section_name, s.year_level, s.schedule_day, s.schedule_time_start, s.schedule_time_end, 
                        ...
                        c.course as curriculum_course, c.year_level as curriculum_year, c.semester as curriculum_semester
                 FROM section s 
                 JOIN subjects sub ON s.subject_id = sub.subject_id 
                 LEFT JOIN curriculum c ON sub.subject_id = c.subject_id AND c.course = ?
...
$stmt->bind_param("sss", $course, $course, $course);
$stmt->execute();
...
// Get curriculum subjects for the student's course
$curriculum_sql = "SELECT c.*, s.subject_code, s.subject_name, s.units, s.description 
                   FROM curriculum c 
                   JOIN subjects s ON c.subject_id = s.subject_id 
                   WHERE c.course = ? 
                   ORDER BY FIELD(c.year_level, '1st Year', '2nd Year', '3rd Year', '4th Year', '5th Year'), c.semester";
$stmt = $conn->prepare($curriculum_sql);
$stmt->bind_param("s", $course);
$stmt->execute();
$curriculum_result = $stmt->get_result();
$curriculum_subjects = [];
while ($row = $curriculum_result->fetch_assoc()) {
    $curriculum_subjects[] = $row;
}
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($conflicts) > 0) {
        ...
        // Insert new schedule with student_id, section_id, and subject_id
...
// Group curriculum by year level
$curriculum_by_year = [
    '1st Year' => [],
    '2nd Year' => [],
    '3rd Year' => [],
...
foreach ($curriculum_subjects as $subject) {
    ...
    if (isset($curriculum_by_year[$year])) {
        $curriculum_by_year[$year][] = $subject;
    }
}
?>
...
<body>
    ...
    <main class="main-content">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <p>Browse all subjects and select the classes you want to enroll in.</p>
        </div>
        
        <div class="curriculum-hint">
          <i class="fas fa-graduation-cap"></i> <strong>Curriculum Guide for <?php echo htmlspecialchars($course); ?></strong><br>
          Below are the subjects from your curriculum. You can also browse all available sections from other tabs.
        </div>
        
        <div class="nav-tabs">
          <button class="nav-tab active" onclick="showTabPane('curriculum')">📖 Curriculum Subjects</button>
          <button class="nav-tab" onclick="showTabPane('all')">📚 All Sections</button>
        </div>
        ...
        <form method="POST" action="" id="scheduleForm" onsubmit="return validateConflictsBeforeSubmit()">
          ...
          <!-- Curriculum Tab Pane -->
          <div id="curriculum-pane" class="tab-pane active">
            <?php if (count($curriculum_subjects) > 0): ?>
              <?php foreach ($curriculum_by_year as $year => $subjects): ?>
                <?php if (!empty($subjects)): ?>
                  <div class="year-section" data-year="<?php echo $year; ?>">
                    <div>
                      <h3 class="year-title">
                        <?php echo $year; ?>
                        <span class="year-badge"><?php echo count($subjects); ?> subjects in curriculum</span>
                      </h3>
                    </div>
                    <div class="sections-grid">
                      <?php foreach ($subjects as $subject): 
                          // Find sections for this subject
                          $matching_sections = array_filter($available_sections, function($section) use ($subject) {
                              return $section['subject_id'] == $subject['subject_id'];
                          });
                          
                          if (empty($matching_sections)) {
                              // Show subject without available sections
                              $is_selected = false;
                      ?>
                              <div class="section-card" style="opacity: 0.6; cursor: default;">
                                <div>
                                  <strong><?php echo htmlspecialchars($subject['subject_code']); ?> - <?php echo htmlspecialchars($subject['subject_name']); ?></strong>
                                  <span class="curriculum-badge">Curriculum</span>
                                </div>
                                <div class="section-details">
                                  <p><i class="fas fa-star"></i> <?php echo $subject['units']; ?> units</p>
                                  <p><i class="fas fa-calendar"></i> <?php echo htmlspecialchars($subject['semester']); ?></p>
                                  <p style="color: #dc3545;"><i class="fas fa-exclamation-circle"></i> No available sections found for this subject</p>
                                </div>
                              </div>
                      <?php
                              continue;
                          ...
                          foreach ($matching_sections as $section):
              ...
              <?php endforeach; ?>
            <?php else: ?>
              <div style="text-align: center; padding: 2rem; color: #8699ad;">
                <i class="fas fa-book-open" style="font-size: 3rem; margin-bottom: 1rem; display: block;"></i>
                <p>No curriculum subjects found for <?php echo htmlspecialchars($course); ?>.</p>
                <p style="font-size: 0.9rem;">Please contact your administrator to set up the curriculum.</p>
              </div>
            <?php endif; ?>
          </div>
          
          <!-- All Sections Tab Pane -->
          <div id="all-pane" class="tab-pane">
              ...
              <?php if (!empty($sections)): ?>
                <div class="year-section" data-year="<?php echo $year; ?>">
                  <div>
                    <h3 class="year-title">
                      <?php echo $year; ?>
                      <span class="year-badge"><?php echo count($sections); ?> sections</span>
                    </h3>
                  ...
                  <div class="sections-grid">
                    ...
                    ?>
                      <div class="section-card <?php echo $is_selected ? 'selected' : ''; ?>" 
                           ...
                           onclick="toggleSection(this, <?php echo $section['section_id']; ?>)">
                        <div>
                          <input type="checkbox" name="sections[]" value="<?php echo $section['section_id']; ?>" id="section_all_<?php echo $section['section_id']; ?>" <?php echo $is_selected ? 'checked' : ''; ?> onclick="event.stopPropagation(); checkConflicts();">
                          <label for="section_all_<?php echo $section['section_id']; ?>" class="section-title">
                            <strong><?php echo htmlspecialchars($section['subject_code']); ?> - <?php echo htmlspecialchars($section['subject_name']); ?></strong>
                          </label>
                        </div>
                        <div class="section-details">
                          ...
                          <p><i class="fas fa-star"></i> <?php echo htmlspecialchars($section['units']); ?> units</p>
                          <p><i class="fas fa-graduation-cap"></i> <strong>Year Level:</strong> <?php echo $year; ?></p>
                          <p><i class="fas fa-code-branch"></i> <strong>Section:</strong> <?php echo htmlspecialchars($section['section_code']); ?></p>
                        </div>
                      </div>
                    <?php endforeach; ?>
                  </div>
          ...
          <div class="form-actions">
            <a href="view_schedule.php" class="cancel-btn">Cancel</a>
  ...
  <script>
    ...
    function selectAllByYear(year) {
      const cards = document.querySelectorAll('#all-pane .section-card[data-year="' + year + '"]');
      cards.forEach(card => {
        const checkbox = card.querySelector('input[type="checkbox"]');
        if (!checkbox.checked) {
    ...
    searchInput.addEventListener('keyup', function() {
      ...
      cards.forEach(card => {
        ...
        if (subjectText && (subjectText.includes(searchTerm) || searchTerm === '')) {
          card.style.display = 'block';
        } else if (!subjectText && searchTerm === '') {
          card.style.display = 'block';
        } else {
